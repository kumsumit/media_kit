...
void main() {
  ...
  test('media-uri-normalization-network', () {
    for (final source in sources.network) {
      final test = source;
      print(test);
      expect(Media.normalizeURI(source), equals(source));
      expect(Media(source).uri, equals(source));
    }
  });
  test(
    ...
    () async {
      ...
      for (final source in sources.file) {
        final test = source;
        print(test);
        expect(Media.normalizeURI(test), equals(source));
        expect(Media(test).uri, equals(source));
      }
      // file:// URI
      for (final source in sources.file) {
  ...
  test('media-extras-propagate', () {
    Media.cache.clear();

    final extras = {'foo': 'bar', 'baz': 'qux'};
    ...
    final c = Media(sources.platform.first, extras: {'x': 'y'});

    print(a.extras);
    print(b.extras);
    print(c.extras);

    expect(MapEquality().equals(a.extras, b.extras), equals(true));
    expect(MapEquality().equals(c.extras, {'x': 'y'}), equals(true));
  });
  test('media-http-headers-propagate', () {
    Media.cache.clear();

    final headers = {for (int i = 0; i < 10; i++) 'key_$i': 'value_$i'};
    ...
    final c = Media(sources.platform.first, httpHeaders: {'x': 'y'});

    print(a.httpHeaders);
    print(b.httpHeaders);
    print(c.httpHeaders);

    expect(MapEquality().equals(a.httpHeaders, b.httpHeaders), equals(true));
    expect(MapEquality().equals(c.httpHeaders, {'x': 'y'}), equals(true));
  }, skip: UniversalPlatform.isWeb);
  test('media-http-headers-propagate', () {
    Media.cache.clear();

    expect(
      () => Media(sources.platform.first, httpHeaders: {'x': 'y'}),
      throwsUnsupportedError,
    );
  }, skip: !UniversalPlatform.isWeb);
  test('media-finalizer', () async {
    Media.cache.clear();
    ...
    for (int i = 0; i < 2; i++) {
      if (i == 0) {
        Media(
          ...
          httpHeaders: {'key_0': 'value_0', 'key_1': 'value_1'},
        );
