...
void main() {
  ...
  test('initializer-create', () {
    expect(Initializer.create(NativeLibrary.path, (_) async {}), completes);
  });
  test('initializer-dispose', () async {
    final handle = await Initializer.create(NativeLibrary.path, (_) async {});
    expect(() => Initializer.dispose(handle), returnsNormally);
  });
  test('initializer-callback', () async {
    ...
    final handle = await Initializer.create(NativeLibrary.path, (event) async {
  ...
  test('initializer-options-with-callback', () async {
    final mpv = MPV(DynamicLibrary.open(NativeLibrary.path));
    ...
    );
    {
      ...
      calloc.free(name);
      expect(value.cast<Utf8>().toDartString(), 'yes');
    }
    {
  ...
  test('initializer-options-without-callback', () async {
    final mpv = MPV(DynamicLibrary.open(NativeLibrary.path));
    final handle = await Initializer.create(
      NativeLibrary.path,
      null,
