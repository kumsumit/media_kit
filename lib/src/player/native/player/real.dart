...
class NativePlayer extends PlatformPlayer {
    ...
    Future<void> function() async {
      ...
      // Enter paused state.
      {
        ...
        if (value.value == 0) {
          ...
          mpv.mpv_command_string(ctx, command.cast());
          // NOTE: Handled as part of [stop] logic.
  ...
  Future<void> stop({bool open = false, bool synchronized = true}) async {
    Future<void> function() async {
      ...
      final commands = ['stop', 'playlist-clear', 'playlist-play-index none'];
      for (final command in commands) {
        final args = command.toNativeUtf8();
        mpv.mpv_command_string(ctx, args.cast());
        calloc.free(args);
      }
      ...
      if (state.playlist.index == index &&
        ...
        state = state.copyWith(
          ...
          playlist: state.playlist.copyWith(
            ...
            index:
                state.playlist.medias.length - 2 < 0
                    ? 0
    ...
    Future<void> function() async {
      ...
      switch (playlistMode) {
        case PlaylistMode.none:
          {
            mpv.mpv_set_property_string(ctx, file.cast(), no.cast());
            mpv.mpv_set_property_string(ctx, playlist.cast(), no.cast());
            break;
          }
        case PlaylistMode.single:
          {
            mpv.mpv_set_property_string(ctx, file.cast(), yes.cast());
            mpv.mpv_set_property_string(ctx, playlist.cast(), no.cast());
            break;
          }
        case PlaylistMode.loop:
          {
            mpv.mpv_set_property_string(ctx, file.cast(), no.cast());
            mpv.mpv_set_property_string(ctx, playlist.cast(), yes.cast());
            break;
          }
      }
      calloc.free(file);
  ...
  Future<void> setRate(double rate, {bool synchronized = true}) {
    Future<void> function() async {
      ...
      if (configuration.pitch) {
        ...
        final no = 'no'.toNativeUtf8();
        mpv.mpv_set_property_string(ctx, name.cast(), no.cast());
        calloc.free(name);
        calloc.free(no);
        ...
        final value =
            'scaletempo:scale=${(state.rate / state.pitch).toStringAsFixed(8)}'
                .toNativeUtf8();
        mpv.mpv_set_property_string(ctx, name.cast(), value.cast());
        calloc.free(name);
        calloc.free(value);
      } else {
